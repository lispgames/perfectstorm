...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defun numbers-to-doubles (tree)
                                        ;traverses a nested list,
                                        ;coercing all numbers into
                                        ;double-floats. useful for
                                        ;the stupid cairo bindings
  ...
  (loop for char-code from 32 to 126 do
       (let* ((surface (cairo:create-image-surface :argb32 width height))
         ...
         (gl:tex-image-2d :texture-2d 0
                          ...
                          (cairo:image-surface-get-data surface)) ;;(get-surface-data-as-array surface 4)
  ...
  (with-gensyms (surface-symbol texture-name-symbol data-symbol)
    `(defmethod make-texture ((type (eql ',type)) width height &optional color)
       ...
       (let* ((,surface-symbol (cairo:create-image-surface :argb32 width height))
         ...
         (let ((,data-symbol (cairo:image-surface-get-data ,surface-symbol)))
           ;; (get-surface-data-as-array ,surface-symbol 4)
           (gl:tex-image-2d :texture-2d 0
                            :rgba width height 0 :bgra :unsigned-byte
                            ,data-symbol)
...
(defun mix-colors (color-with-factor color) ;colors represented as (r
                                        ;g b a factor) and (r g b a).
                                        ;make classes for these?
...
;;
;;
;; the secrets of the gradient color matrix (or of mixed colors in
;; general)
;; '((offset r g b a factor)
;;   ( ...                 ))
;; offset:  [0..1] location of the color stop within the gradient
;; r,g,b,a: base color
;; factor:  [-1..1] argument color is multiplied by this factor and
;; then added to the base color
;;
...
(def-texture-method air-unit
    (let ((exhaust-scale 0.5d0))
      ...
      (with-gradient (ground-unit-gradient :radial ((* width 0.7) (/ height 2)
        ...
        (cairo:translate 0.0d0 (* height  0.5)); (/
                                        ; exhaust-scale))))
...
(def-texture-method full-health-element
  ...
  (relative-path width height cairo:*context*
                 '((0 0)
                   (0 1)
                   (1 0.7)
                   (1 0.3))
...
(def-texture-method factory
    (let ((path    '((0 0)
      ...
      (relative-path width height cairo:*context* path :border (* 3 *relative-line-width*))
      (cairo:stroke)))
...
;;;;;;;;;;;;;;;;;;
;;
;; mip mapping. the following code is redundant since cl-opengl-thomas
;;has been patched to
;; support mip mapping. but i can't delete it, it makes me sad :(
;;


(defun shrink (width height data) ;shrink an rgba image of the given
                                        ;size by one half in each dimension
...
(defun make-mipmaps (width height data &key (level 1)) ;creates and
                                        ;stores mipmaps of a texture.
                                        ;assumes texture is already
                                        ;bound
  (when (and
         (> (round width) 1)
         (> (round height) 1))
